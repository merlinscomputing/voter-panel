...
void I2C2_Initialize(void) {
    ...
    // Calculated Frequency: 394,089 Hz
    I2C2BRG = 0x0050;//0x0026;
    // RCEN disabled; A10M 7 Bit; RSEN disabled; STRICT disabled; ACKEN disabled; DISSLW enabled; SCLREL Holds; SEN disabled; ACKDT Sends ACK; GCEN disabled; SMEN disabled; PEN disabled; I2CEN enabled; I2CSIDL disabled; STREN disabled; 
    I2C2CONL = 0x8200;
...
void DRV_I2C2_MasterWrite(
        uint8_t *pdata,
        uint8_t length,
        uint16_t address,
        DRV_I2C2_MESSAGE_STATUS *pstatus) {
    I2C2_MasterWrite(pdata, length, address, (I2C2_MESSAGE_STATUS *) pstatus);
}
...
void DRV_I2C2_MasterRead(
        uint8_t *pdata,
        uint8_t length,
        uint16_t address,
        DRV_I2C2_MESSAGE_STATUS *pstatus) {
    I2C2_MasterRead(pdata, length, address, (I2C2_MESSAGE_STATUS *) pstatus);
}
...
void DRV_I2C2_MasterTRBInsert(
        uint8_t count,
        DRV_I2C2_TRANSACTION_REQUEST_BLOCK *ptrb_list,
        DRV_I2C2_MESSAGE_STATUS *pflag) {
    I2C2_MasterTRBInsert(count, (I2C2_TRANSACTION_REQUEST_BLOCK *) ptrb_list, (I2C2_MESSAGE_STATUS *) pflag);
}
...
void DRV_I2C2_MasterReadTRBBuild(
        DRV_I2C2_TRANSACTION_REQUEST_BLOCK *ptrb,
        uint8_t *pdata,
        uint8_t length,
        uint16_t address) {
    I2C2_MasterReadTRBBuild((I2C2_TRANSACTION_REQUEST_BLOCK *) ptrb, pdata, length, address);
}
...
void DRV_I2C2_MasterWriteTRBBuild(
        DRV_I2C2_TRANSACTION_REQUEST_BLOCK *ptrb,
        uint8_t *pdata,
        uint8_t length,
        uint16_t address) {
    I2C2_MasterWriteTRBBuild((I2C2_TRANSACTION_REQUEST_BLOCK *) ptrb, pdata, length, address);
}
...
bool DRV_I2C2_MasterQueueIsEmpty(void) {
    return I2C2_MasterQueueIsEmpty();
}
...
bool DRV_I2C2_MasterQueueIsFull(void) {
    return I2C2_MasterQueueIsFull();
}

/*
void I2C2_Char_Send_LCD(uint8_t writeBuffer)
{
   ...
   // uint8_t *pData;
    ...
    //I2C_MESSAGE_STATUS status = I2C2_MESSAGE_PENDING;
    //I2C_MESSAGE_PENDING;

    slaveDeviceAddress = 0x43; // starting EEPROM address 
   //dataAddress = 0x10;
   ...
   // for (counter = 0; counter < nCount; counter++) {
      ...
      //  writeBuffer[2] = *pD++;
        ...
        while (status != I2C2_MESSAGE_FAIL) {
            ...
            I2C2_MasterWrite(writeBuffer, 1, slaveDeviceAddress, &status);
...
void SendI2C2(char addr,char data)
{
//#define LCDAddress = 0x43;
    //I2C1CON1bits.SEN =1;
    I2C2CONLbits.SEN=1;
    ...
    //I2C2TRN = 0x43;
    //WaitMSSP();
    while(I2C2STATbits.ACKSTAT==1);//   SSPCON2bits.ACKSTAT == 1);
    I2C2TRN = addr; // send register address to MCP7940
    //delay_ms(5);// WaitMSSP();
    while(I2C2STATbits.ACKSTAT == 1);
    I2C2TRN = data; // send register address to MCP7940
    //WaitMSSP();
...
/*
char ReadI2C(char addr)
{
    char result;
    
    SSPCON2bits.RSEN = 1; // send restart
    while(RSEN); // wait for transmission to complete
    PIR1bits.SSP1IF = 0; // clear interrupt flag
    SSPBUF = 0b11011110; // send control byte to MCP7940 - write
    WaitMSSP();
    while(SSPCON2bits.ACKSTAT == 1); // wait for RTC to be ready
    SSPBUF = addr; // send register address to MCP7940
    WaitMSSP();
    while(SSPCON2bits.ACKSTAT == 1); // wait for RTC to be ready
    SSPCON2bits.RSEN = 1; // send restart
    WaitMSSP();
    SSPBUF = 0b11011111; // send control byte to MCP7940 - read
    WaitMSSP();
    while(SSPCON2bits.ACKSTAT == 1); // wait for RTC to be ready
    SSPCON2bits.RCEN = 1; // put master in receive mode
    while(RCEN); // wait for transmission to complete
    PIR1bits.SSP1IF = 0; // clear interrupt flag
    SSPCON2bits.ACKDT = 1; // NACK
    SSPCON2bits.ACKEN =1; // send NACK now
    while(SSPCON2bits.ACKEN); // wait for NACK send to complete
    SSPCON2bits.PEN = 1; // send STOP
    while(PEN);
    PIR1bits.SSP1IF = 0; // clear interrupt flag
    result = SSPBUF; // get byte that was sent by RTC
    return result; // and return it to caller
}
 
void WaitMSSP()
{
    while(PIR1bits.SSP1IF == 0); // SSP1IF is set when operation complete
    {
        PIR1bits.SSP1IF = 0;    // clear interrupt flag
    }
}
